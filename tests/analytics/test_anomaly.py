...
class TestDetectRmssdAnomaly:
    ...
    def test_detect_anomaly_simple_method(self, normal_hrv_features, baseline_7days):
        ...
        result = detect_rmssd_anomaly(normal_hrv_features, baseline_7days, method="simple")
    ...
    def test_detect_anomaly_zscore_method(self, normal_hrv_features, baseline_7days):
        ...
        result = detect_rmssd_anomaly(normal_hrv_features, baseline_7days, method="zscore")
    ...
    def test_detect_anomaly_rolling_method(self, normal_hrv_features, baseline_7days):
        ...
        result = detect_rmssd_anomaly(normal_hrv_features, baseline_7days, method="rolling")
    ...
    def test_detect_anomaly_invalid_method(self, normal_hrv_features, baseline_7days):
        ...
        with pytest.raises(ValueError):
            detect_rmssd_anomaly(normal_hrv_features, baseline_7days, method="invalid_method")

    def test_detect_anomaly_normal_vs_abnormal(self, normal_hrv_features, poor_hrv_features, baseline_7days):
        """Test that abnormal RMSSD is detected differently."""
        result_normal = detect_rmssd_anomaly(normal_hrv_features, baseline_7days, method="zscore")
        result_abnormal = detect_rmssd_anomaly(poor_hrv_features, baseline_7days, method="zscore")
        
        # Both should have results but likely different statuses
        assert result_normal.get("status") in ["normal", "low", "high", "insufficient_data"]
        assert result_abnormal.get("status") in ["normal", "low", "high", "insufficient_data"]
...
class TestZscoreMethod:
    ...
    def test_zscore_insufficient_data(self, normal_hrv_features, baseline_insufficient_data):
    ...
    def test_zscore_normal_value(self, normal_hrv_features, baseline_30days):
        ...
        features = [
            HRVFeatures(date=f"2026-05-{i:02d}", rmssd=60.0, mean_hr=70.0, sdnn=80.0, pnn50=25.0,
                       ln_rmssd=4.09, vlf=500, lf=1500, hf=2000, lf_hf=0.75, hf_pct=0.4, lf_nu=0.4, hf_nu=0.6)
            for i in range(1, 8)
        ]
        baseline = Baseline(history=features)
        
        current = HRVFeatures(date="2026-05-08", rmssd=60.0, mean_hr=70.0, sdnn=80.0, pnn50=25.0,
                             ln_rmssd=4.09, vlf=500, lf=1500, hf=2000, lf_hf=0.75, hf_pct=0.4, lf_nu=0.4, hf_nu=0.6)
...
class TestRollingMethod:
    ...
    def test_rolling_returns_baseline_and_delta(self, normal_hrv_features, baseline_30days):
    ...
    def test_rolling_insufficient_data(self, normal_hrv_features, baseline_insufficient_data):
...
class TestAnomalyDetectionIntegration:
    ...
    def test_all_methods_with_same_input(self, normal_hrv_features, baseline_30days):
        ...
        assert all(isinstance(r, dict) for r in [result_simple, result_zscore, result_rolling])
        assert all("status" in r for r in [result_simple, result_zscore, result_rolling])
    ...
    def test_anomaly_detection_consistency(self, normal_hrv_features, baseline_7days):
        ...
        result1 = detect_rmssd_anomaly(normal_hrv_features, baseline_7days, method="simple")
        result2 = detect_rmssd_anomaly(normal_hrv_features, baseline_7days, method="simple")
