...
class TestTrendRmssd:
    ...
    def test_trend_rmssd_increasing_trend(self):
        ...
        features = [
            HRVFeatures(date=f"2026-05-{i:02d}", rmssd=40.0 + i * 10, mean_hr=70.0, sdnn=80.0, pnn50=25.0,
    ...
    def test_trend_rmssd_decreasing_trend(self):
        ...
        features = [
            HRVFeatures(date=f"2026-05-{i:02d}", rmssd=100.0 - i * 10, mean_hr=70.0, sdnn=80.0, pnn50=25.0,
                       ln_rmssd=4.09, vlf=500, lf=1500, hf=2000, lf_hf=0.75, hf_pct=0.4, lf_nu=0.4, hf_nu=0.6)
            for i in range(1, 8)
        ]
        baseline = Baseline(history=features)
    ...
    def test_trend_rmssd_exact_five_points(self):
        ...
        features = [
            HRVFeatures(date=f"2026-05-0{i}", rmssd=50.0 + i * 5, mean_hr=70.0, sdnn=80.0, pnn50=25.0,
                       ln_rmssd=4.09, vlf=500, lf=1500, hf=2000, lf_hf=0.75, hf_pct=0.4, lf_nu=0.4, hf_nu=0.6)
            for i in range(1, 6)
        ]
        baseline = Baseline(history=features)
    ...
    def test_trend_rmssd_with_noise(self):
        ...
        features = [
            HRVFeatures(date=f"2026-05-{i:02d}", rmssd=max(10, base_values[i]), mean_hr=70.0, sdnn=80.0, pnn50=25.0,
                       ln_rmssd=4.09, vlf=500, lf=1500, hf=2000, lf_hf=0.75, hf_pct=0.4, lf_nu=0.4, hf_nu=0.6)
            for i in range(10)
        ]
        baseline = Baseline(history=features)
...
class TestTrendsIntegration:
    ...
    def test_trend_detection_full_workflow(self):
        ...
        if result.get("status") != "insufficient_data":
            assert result["slope"] > 0
            assert result["trend"] in ["increasing", "stable"]  # Small improvements might be stable
    ...
    def test_trend_week_vs_month(self):
        ...
        # 30 days data
        features_30d = [
            HRVFeatures(date=f"2026-04-{13 + (i%30):02d}", rmssd=50.0 + i * 0.5, mean_hr=70.0, sdnn=80.0, pnn50=25.0,
                       ln_rmssd=4.09, vlf=500, lf=1500, hf=2000, lf_hf=0.75, hf_pct=0.4, lf_nu=0.4, hf_nu=0.6)
            for i in range(30)
        ]
    ...
    def test_trend_with_recovery_pattern(self):
        ...
        features = [
            HRVFeatures(date=f"2026-05-{i:02d}", rmssd=rmssd_values[i], mean_hr=70.0, sdnn=80.0, pnn50=25.0,
                       ln_rmssd=4.09, vlf=500, lf=1500, hf=2000, lf_hf=0.75, hf_pct=0.4, lf_nu=0.4, hf_nu=0.6)
