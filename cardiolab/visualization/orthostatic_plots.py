...
def plot_orthostatic_phases_evolution(
    ...
    * **LF/HF autonomic balance** — hf_hr_pct_change and lf_hr_pct_change (%)
    ...
    ax_rmssd.plot(x, rmssd_sup, color=_SUPINE_COLOR,
                  label=lbl(labels, "_phase_Supine", "Supine"), **mk)
    ax_rmssd.plot(x, rmssd_tra, color=_TRANSITION_COLOR,
                  label=lbl(labels, "_phase_Transition", "Transition"), **mk, linestyle="--")
    ax_rmssd.plot(x, rmssd_sta, color=_STANDING_COLOR,
                  label=lbl(labels, "_phase_Standing", "Standing"), **mk)
    ...
    width = 0.35
    ax_resp.bar(x - width / 2, hr_resp, width, color=_DARK, alpha=0.75,
                label=lbl(labels, "hr_response", "ΔHR (bpm)"))
    ax_resp_r = ax_resp.twinx()
    ax_resp_r.plot(x, d_rmssd, color=_DELTA_COLOR, linewidth=1.8, marker="s", markersize=5,
                   label=lbl(labels, "delta_rmssd", "ΔRMSSD (ms)"))
    ax_resp.set_ylabel(lbl(labels, "hr_response", "ΔHR (bpm)"), fontsize=10)
    ax_resp_r.set_ylabel(lbl(labels, "delta_rmssd", "ΔRMSSD (ms)"), fontsize=10,
                         color=_DELTA_COLOR)
    ...
    ax_balance.plot(x, hf_hr_pct, color=_SUPINE_COLOR, linewidth=1.8, marker="o", markersize=5,
                    label=lbl(labels, "hf_hr_pct_change", "HF/HR Δ (%)"))
    ax_balance.plot(x, lf_hr_pct, color=_STANDING_COLOR, linewidth=1.8, marker="^", markersize=5,
